...
$smarty->assign('languages', $_CFG['lang']);
...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
    ...
    /* 上一篇下一篇文章 */
    $next_article = $db->getRow("SELECT article_id,  ".$GLOBALS['title_field']." as title FROM " .$ecs->table('article'). " WHERE article_id > $article_id AND cat_id=$article[cat_id] AND is_open=1 LIMIT 1");
    ...
    if (!empty($prev_aid))
    {
        $prev_article = $db->getRow("SELECT article_id, ".$GLOBALS['title_field']." as title FROM " .$ecs->table('article'). " WHERE article_id = $prev_aid");
...
{
    /* 获得文章的信息 */
    $sql = "SELECT a.*, ".$GLOBALS['title_field']." as title, a.".$GLOBALS['content_field']." as content, IFNULL(AVG(r.comment_rank), 0) AS comment_rank ".
