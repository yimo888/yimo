...
function get_categories_tree($cat_id = 0)
{
    ...
    */
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        $sql = 'SELECT c.cat_id,'.$GLOBALS['cat_name_field'].' as cat_name ,parent_id,is_show,r.recommend_type as is_hot ' .
                'FROM ' . $GLOBALS['ecs']->table('category') . " as c ".
                 " left join " . $GLOBALS['ecs']->table('cat_recommend') . "  as r on r.cat_id=c.cat_id and recommend_type=3 ".
                "WHERE parent_id = '$parent_id' AND is_show = 1 group by c.cat_id ORDER BY sort_order ASC, cat_id ASC";
...
function get_child_tree($tree_id = 0)
{
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        ...
        $child_sql = 'SELECT c.cat_id,  '.$GLOBALS['cat_name_field'].' as cat_name, parent_id, is_show,r.recommend_type as is_hot ' .

                'FROM ' . $GLOBALS['ecs']->table('category') . " as c ".
                 " left join " . $GLOBALS['ecs']->table('cat_recommend') . "  as r on r.cat_id=c.cat_id and recommend_type=3 ".
                "WHERE parent_id = '$tree_id' AND is_show = 1 group by c.cat_id ORDER BY sort_order ASC, cat_id ASC";
...
function get_top10($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.'.$GLOBALS['goods_name_field'].' as goods_name, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number ' .
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.'.$GLOBALS['goods_name_field'].' as goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
...
{
    $time = gmtime();
    $sql = 'SELECT g.*,g.'.$GLOBALS['goods_name_field'].' as goods_name,g.'.$GLOBALS['goods_desc_field'].' as goods_desc,g.'.$GLOBALS['seller_note_field'].' as seller_note,IF(g.give_integral > -1, g.give_integral, g.shop_price) as give_integral,c.'.$GLOBALS['measure_unit_field'].' AS measure_unit, c.cat_name, b.brand_id, b.'.$GLOBALS['brand_name_field'].' AS goods_brand, m.type_money AS bonus_money, ' .
...
function get_goods_properties($goods_id)
{
    ...
    /* 获得商品的规格 */
    $sql = "SELECT a.attr_id,a.".$GLOBALS['attr_name_field']." as attr_name, a.attr_group, a.is_linked, a.attr_type, ".
                "g.goods_attr_id, g.".$GLOBALS['attr_value_field']." as attr_value, g.attr_price " .
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.'.$GLOBALS['goods_name_field'].' as goods_name, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
    ...
    /* 分类信息 */
    $sql = 'SELECT '.$GLOBALS['cat_name_field'].' as cat_name FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
...
function group_buy_info($group_buy_id, $current_num = 0)
{
    ...
    $sql = "SELECT *, act_id AS group_buy_id, ".$GLOBALS['act_desc_field']." AS group_buy_desc, start_time AS start_date, end_time AS end_date " .
...
{
    $sql = "SELECT *, ".$GLOBALS['act_name_field']." as act_name, ".$GLOBALS['act_desc_field']." as act_desc FROM " . $GLOBALS['ecs']->table('goods_activity') . " WHERE act_id = '$act_id'";
...
{
    $sql = "SELECT g.*,g.".$GLOBALS['goods_name_field']." as goods_name, b.".$GLOBALS['brand_name_field']." as brand_name " .
            "FROM " . $GLOBALS['ecs']->table('goods') . " AS g " .
                "LEFT JOIN " . $GLOBALS['ecs']->table('brand') . " AS b ON g.brand_id = b.brand_id " .
            "WHERE g.goods_id = '$goods_id'";
    $row = $GLOBALS['db']->getRow($sql);
