...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    /* 初始化“页面标题”和“当前位置” */
    $page_title = $GLOBALS['_CFG']['shop_title'] . ' - ' . 'Powered by Igo';
...
function get_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, '.$GLOBALS['cat_name_field'].' as cat_name, parent_id FROM ' . $GLOBALS['ecs']->table('category'));
...
{
    $sql = 'SELECT c.cat_id, c.'.$GLOBALS['cat_name_field'].' as cat_name , c.sort_order, a.article_id, a.'.$GLOBALS['title_field'].' as title , a.file_url, a.open_type ' .
...
function get_article_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, '.$GLOBALS['cat_name_field'].' as cat_name, parent_id FROM ' . $GLOBALS['ecs']->table('article_cat'));
...
{
    $sql = 'SELECT *, '.$GLOBALS['name_field'].' as name FROM '. $GLOBALS['ecs']->table('nav') . '
