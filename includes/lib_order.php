...
{
	if(!isset($GLOBALS['shipping_name_field']))
	{
		$GLOBALS['shipping_name_field'] = 'shipping_name';
	}
    if(!isset($GLOBALS['shipping_desc_field']))
	{
		$GLOBALS['shipping_desc_field'] = 'shipping_desc';
	}
  


    $sql = 'SELECT s.shipping_id, s.shipping_code,s.'.$GLOBALS['shipping_name_field'].' as shipping_name, ' .
                's.'.$GLOBALS['shipping_desc_field'].' as shipping_desc, s.insure, s.support_cod, a.configure ' .
...
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false)
{
    ...
    if(!isset($GLOBALS['pay_name_field']))
	{
		$GLOBALS['pay_name_field'] = 'pay_name';
	}
    if(!isset($GLOBALS['pay_desc_field']))
	{
		$GLOBALS['pay_desc_field'] = 'pay_desc';
	}

    $sql = 'SELECT pay_id, pay_code,'.$GLOBALS['pay_name_field'].' as pay_name, pay_fee, '.$GLOBALS['pay_desc_field'].' as pay_desc, pay_config, is_cod' .
            ' FROM ' . $GLOBALS['ecs']->table('payment') .
            ' WHERE enabled = 1 ';
    if (!$support_cod)
    {
...
function pack_list()
{
    $sql = 'SELECT *,'.$GLOBALS['pack_name_field'].' as pack_name,'.$GLOBALS['pack_desc_field'].' as pack_desc FROM ' . $GLOBALS['ecs']->table('pack');
    $res = $GLOBALS['db']->query($sql);
...
function card_list()
{
    $sql = "SELECT *,".$GLOBALS['card_name_field']." as card_name,".$GLOBALS['card_desc_field']." as card_desc FROM " . $GLOBALS['ecs']->table('card');
    $res = $GLOBALS['db']->query($sql);
...
function order_goods($order_id)
{
			...
			"og.goods_price * og.goods_number AS subtotal, og.extension_code,g.goods_thumb ,og.goods_attr_id,g.goods_thumb " .
			 "FROM " . $GLOBALS['ecs']->table('order_goods') . 
			 " AS og "." LEFT JOIN ". $GLOBALS['ecs']->table('goods') . 
			 " AS g on og.goods_id=g.goods_id " .
			 " WHERE order_id = '$order_id'order by g.goods_sn";
    $res = $GLOBALS['db']->query($sql);
...
{
    $sql = "SELECT rec_id, user_id, c.goods_id, g.".$GLOBALS['goods_name_field']." as goods_name, c.goods_sn, c.goods_number, " .
            "c.market_price, goods_price, goods_attr_id, goods_attr,c. is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, " .
            "goods_price * c.goods_number AS subtotal " .
...
function get_goods_attr_info($arr, $type = 'pice')
{
    ...
    if (!empty($arr))
    {
        ...
        $sql = "SELECT a.".$GLOBALS['attr_name_field']." as attr_name, ga.".$GLOBALS['attr_value_field']." as attr_value, ga.attr_price ".
...
function user_bonus($user_id, $goods_amount = 0)
{
    ...
    $sql = "SELECT t.type_id, t.".$GLOBALS['type_name_field']." as type_name, t.type_money, b.bonus_id " .
...
{
    $sql = "SELECT t.*, t.".$GLOBALS['type_name_field']." as type_name, b.* " .
            "FROM " . $GLOBALS['ecs']->table('bonus_type') . " AS t," .
                $GLOBALS['ecs']->table('user_bonus') . " AS b " .
            "WHERE t.type_id = b.bonus_type_id ";
    if ($bonus_id > 0)
...
function get_cart_goods($rec_type = CART_GENERAL_GOODS)
{
    ...
    /* 循环、统计 */
    $sql = "SELECT c.*, g.".$GLOBALS['goods_name_field']." as goods_name, IF(c.parent_id, c.parent_id, c.goods_id) AS pid " .
            " FROM " . $GLOBALS['ecs']->table('cart') . " as c " .
			" left join ". $GLOBALS['ecs']->table('goods') . " as g on g.goods_id=c.goods_id ".
            " WHERE c.session_id = '" . SESS_ID . "' AND c.rec_type = '" . $rec_type . "'" .
...
function compute_discount()
{
    ...
    $sql = "SELECT *, ".$GLOBALS['act_name_field']." as act_name " .
