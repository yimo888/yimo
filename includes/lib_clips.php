...
function get_rank_info()
{
    ...
    {
        $sql = "SELECT ".$GLOBALS['rank_name_field']." as rank_name, special_rank FROM " . $ecs->table('user_rank') . " WHERE rank_id = '$_SESSION[user_rank]'";
        $row = $db->getRow($sql);
        if (empty($row))
        {
            return array();
        ...
        else
        {
            ...
            $sql = "SELECT ".$GLOBALS['rank_name_field']." as rank_name,min_points FROM " . $ecs->table('user_rank') . " WHERE min_points > '$user_rank' ORDER BY min_points ASC LIMIT 1";
            $rt  = $db->getRow($sql);
