...
function db_create_no_in($item_list, $field_name = '')
{
    if (empty($item_list))
    {
...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.cat_name_en, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
    ...
    if ($re_type == true)
    {
        ...
        foreach ($options AS $var)
        {
            ...
            $select .= htmlspecialchars(addslashes($var['cat_name']), ENT_QUOTES) . ' '.$var['cat_name_en'] . '</option>';
        }
            ...
            while (!empty($arr))
            {
                foreach ($arr AS $key => $value)
                {
                    ...
                    if ($level == 0 && $last_cat_id == 0)
                    {
						...
						$options[$cat_id]['cat_name_en']  = $value['cat_name_en'];
                        unset($arr[$key]);
...
{
    $arr = array();
    $data = read_static_cache('shop_config1');
    if ($data === false)
    {
        $sql = 'SELECT code, value FROM ' . $GLOBALS['ecs']->table('shop_config_en') . ' WHERE parent_id > 0';
        $res = $GLOBALS['db']->getAll($sql);
        foreach ($res AS $row)
        {
            $arr[$row['code']] = $row['value'];
        ...
        write_static_cache('shop_config1', $arr);
    }
    else
    {
        $arr = $data;
...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    $sql = "SELECT b.brand_id, b.".$GLOBALS['brand_name_field']." as brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
...
function get_promotion_info($goods_id = '')
{
    ...
    $gmtime = gmtime();
    $sql = 'SELECT act_id, '.$GLOBALS['act_name_field'].' as act_name, act_type, start_time, end_time FROM ' . $GLOBALS['ecs']->table('goods_activity') . " WHERE is_finished=0 AND start_time <= '$gmtime' AND end_time >= '$gmtime'";
    if(!empty($goods_id))
    {
    ...
    $favourable = array();
    $sql = 'SELECT act_id, act_range, act_range_ext, '.$GLOBALS['act_name_field'].' as act_name, start_time, end_time FROM ' . $GLOBALS['ecs']->table('favourable_activity') . " WHERE start_time <= '$gmtime' AND end_time >= '$gmtime'";
    if(!empty($goods_id))
    {
...
function get_package_info($id)
{
    ...
    $sql = "SELECT act_id AS id,  act_name AS package_name,  act_name_en, goods_id , goods_name, start_time, end_time, act_desc, act_desc_en, ext_info".
        " FROM " . $GLOBALS['ecs']->table('goods_activity') .
