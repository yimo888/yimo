...
function get_cat_articles($cat_id, $page = 1, $size = 20 ,$requirement='')
{
    ...
    if ($requirement != '')
    {
        $sql = 'SELECT article_id, '.$GLOBALS['title_field'].' as title, author, add_time, file_url, open_type,a.'.$GLOBALS['description_field'].' as description,c.'.$GLOBALS['cat_name_field'].' as cat_name,c.cat_id,title_pic ' .
