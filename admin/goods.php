...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        if ($_REQUEST['act'] == 'copy')
        {
            ...
            $sql = "SELECT 0 AS goods_id, attr_id, attr_value, attr_value_en, attr_price " .
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            ...
            "cat_id, brand_id, shop_price, market_price, virtual_sales, is_promote, promote_price, " .
                    ...
                    "seller_note,seller_note_en,goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
        ...
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, virtual_sales, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, seller_note_en, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$virtual_sales', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$_POST[seller_note_en]', '$goods_weight', '$goods_number',".
	...
	$db->query("update " . $ecs->table('goods') . " set goods_name_en='$_POST[goods_name_en]',goods_desc_en='$_POST[goods_desc_en]',goods_brief_en='$_POST[goods_brief_en]',keywords_en='$_POST[keywords_en]' where goods_id=$goods_id");
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        ...
        if(isset($_POST['attr_id_list']))
        {
            foreach ($_POST['attr_id_list'] AS $key => $attr_id)
            {
				...
				$attr_value_en = $_POST['attr_value_en_list'][$key];
                ...
                if (!empty($attr_value))
                {
                    if (isset($goods_attr_list[$attr_id][$attr_value]))
                    {
						...
						$goods_attr_list[$attr_id][$attr_value]['attr_value_en'] = $attr_value_en;
												$goods_attr_list[$attr_id][$attr_value]['attr_gallery_id'] = $attr_gallery_id;
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                if ($info['sign'] == 'insert')
                {
                    $sql = "INSERT INTO " .$ecs->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_value_en, attr_price)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_value_en]', '$info[attr_price]')";
                }
                elseif ($info['sign'] == 'update')
                {
                    $sql = "UPDATE " .$ecs->table('goods_attr'). " SET attr_price = '$info[attr_price]',attr_value_en = '$info[attr_value_en]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                }
                else
                {
...
elseif ($_REQUEST['act'] == 'edit_goods_name_en')
{
    check_authz_json('goods_manage');
    ...
    if ($exc->edit("goods_name_en = '$goods_name', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result(stripslashes($goods_name));
    }
