...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($_POST['media_type'] == '0')
    {
		...
		if ((isset($_FILES['ad_img_en']['error']) && $_FILES['ad_img_en']['error'] == 0) || (!isset($_FILES['ad_img_en']['error']) && isset($_FILES['ad_img_en']['tmp_name'] ) &&$_FILES['ad_img_en']['tmp_name'] != 'none'))
        {
            $ad_code = basename($image->upload_image($_FILES['ad_img_en'], 'afficheimg'));
        }
        if (!empty($_POST['img_url_en']))
        {
            $ad_code = $_POST['img_url_en'];
        }
        if (((isset($_FILES['ad_img_en']['error']) && $_FILES['ad_img_en']['error'] > 0) || (!isset($_FILES['ad_img_en']['error']) && isset($_FILES['ad_img_en']['tmp_name']) && $_FILES['ad_img_en']['tmp_name'] == 'none')) && empty($_POST['img_url_en']))
        {
            $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
            sys_msg($_LANG['js_languages']['ad_photo_empty'], 0, $link);
        }
    ...
    /* 插入数据 */
    $sql = "INSERT INTO ".$ecs->table('ad'). " (position_id,media_type,ad_name,ad_link,ad_code,ad_code_en,start_time,end_time,link_man,link_email,link_phone,click_count,enabled)
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($type == 0)
    {
		...
		if ((isset($_FILES['ad_img_en']['error']) && $_FILES['ad_img_en']['error'] == 0) || (!isset($_FILES['ad_img_en']['error']) && isset($_FILES['ad_img_en']['tmp_name']) && $_FILES['ad_img_en']['tmp_name'] != 'none'))
        {
            $img_up_info = basename($image->upload_image($_FILES['ad_img_en'], 'afficheimg'));
            $ad_code_en = "ad_code_en = '".$img_up_info."'".',';
        }
        else
        {
            $ad_code_en = '';
        ...
        if (!empty($_POST['img_url_en']))
        {
            $ad_code_en = "ad_code_en = '$_POST[img_url_en]', ";
        }
