...
if ($_REQUEST['act'] == 'edit_type_name_en')
{
    check_authz_json('bonus_manage');
    ...
    $val = json_str_iconv(trim($_POST['val']));
    /* 检查红包类型名称是否重复 */
    if (!$exc->is_only('type_name_en', $id, $val))
    {
        make_json_error($_LANG['type_name_exist']);
    }
    else
    {
        $exc->edit("type_name_en='$val'", $id);
        make_json_result(stripslashes($val));
    }
}
/*------------------------------------------------------ */
...
if ($_REQUEST['act'] == 'insert')
{
	...
	$type_name_en   = !empty($_POST['type_name_en']) ? trim($_POST['type_name_en']) : '';
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name, type_name_en, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount)
    VALUES ('$type_name','$type_name_en',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
            '$use_startdate',
