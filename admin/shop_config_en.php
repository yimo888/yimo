 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: shop_config_en.php 17217 2011-01-19 06:29:08Z liubo $
 */
...
if ($_REQUEST['act'] == 'list_edit')
{
    ...
    assign_query_info();
    $smarty->display('shop_config_en.htm');
}
...
elseif ($_REQUEST['act'] == 'post')
{
    ...
    $arr = array();
    $sql = 'SELECT id, value FROM ' . $ecs->table('shop_config_en');
    $res= $db->query($sql);
    ...
    foreach ($_POST['value'] AS $key => $val)
    {
        if($arr[$key] != $val)
        {
            $sql = "UPDATE " . $ecs->table('shop_config_en') . " SET value = '" . trim($val) . "' WHERE id = '" . $key . "'";
            $db->query($sql);
        }
    }
    ...
    /* 处理上传文件 */
    $file_var_list = array();
    $sql = "SELECT * FROM " . $ecs->table('shop_config_en') . " WHERE parent_id > 0 AND type = 'file'";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
    ...
    foreach ($_FILES AS $code => $file)
    {
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
                ...
                {
                    $sql = "UPDATE " . $ecs->table('shop_config_en') . " SET value = '$file_name' WHERE code = '$code'";
                    $db->query($sql);
                }
                else
                {
    ...
    if (!empty($_POST['invoice_rate']))
    {
        ...
        $sql = "UPDATE " . $ecs->table('shop_config_en') . " SET value = '" . serialize($invoice) . "' WHERE code = 'invoice_type'";
        $db->query($sql);
    }
    ...
    {
        $links[] = array('text' => $_LANG['back_mail_settings'], 'href' => 'shop_config_en.php?act=mail_settings');
        sys_msg($_LANG['mail_save_success'].$spt, 0, $links);
    }
    else
    {
        $links[] = array('text' => $_LANG['back_shop_config'], 'href' => 'shop_config_en.php?act=list_edit');
...
function get_settings($groups=null, $excludes=null)
{
    ...
    /* 取出全部数据：分组和变量 */
    $sql = "SELECT * FROM " . $ecs->table('shop_config_en') .
