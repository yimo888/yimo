...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /*插入数据*/
    $sql = "INSERT INTO ".$ecs->table('card')."(card_name, card_name_en, card_fee, free_money, card_desc, card_desc_en, card_img)
            VALUES ('$_POST[card_name]','$_POST[card_name_en]', '$_POST[card_fee]', '$_POST[free_money]', '$_POST[card_desc]', '$_POST[card_desc_en]', '$img_name')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT * FROM " .$ecs->table('card'). " WHERE card_id='$_REQUEST[id]'";
    $card = $db->GetRow($sql);
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $param = "card_name = '$_POST[card_name]',card_name_en = '$_POST[card_name_en]', card_fee = '$_POST[card_fee]', free_money= $_POST[free_money], card_desc = '$_POST[card_desc]', card_desc_en = '$_POST[card_desc_en]'";
    /* 处理图片 */
...
elseif ($_REQUEST['act'] == 'edit_card_name_en')
{
    check_authz_json('card_manage');
    $card_id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
    $card_name_en = empty($_REQUEST['val']) ? '' : json_str_iconv(trim($_REQUEST['val']));

    if ($exc->edit("card_name_en='$card_name_en'", $card_id))
    {
        make_json_result(stripcslashes($card_name_en));
    }
    else
    {
        make_json_error($db->error());
