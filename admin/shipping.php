...
if ($_REQUEST['act'] == 'list')
{
    ...
    for ($i = 0; $i < count($modules); $i++)
    {
        ...
        /* 检查该插件是否已经安装 */
        $sql = "SELECT shipping_id, shipping_name, shipping_name_en, shipping_desc, insure, support_cod,shipping_order FROM " .$ecs->table('shipping'). " WHERE shipping_code='" .$modules[$i]['code']. "' ORDER BY shipping_order";
        $row = $db->GetRow($sql);
        ...
        if ($row)
        {
			...
			$modules[$i]['name_en']    = $row['shipping_name_en'];
...
elseif ($_REQUEST['act'] == 'edit_name_en')
{
    /* 检查权限 */
    check_authz_json('ship_manage');
    ...
    /* 更新支付方式名称 */
    $exc->edit("shipping_name_en = '$val'", $id);
    make_json_result(stripcslashes($val));
}
