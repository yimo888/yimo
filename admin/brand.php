...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /*插入数据*/
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, brand_name_en, site_url, brand_desc, brand_desc_en, brand_logo, is_show, sort_order) ".
           "VALUES ('$_POST[brand_name]','$_POST[brand_name_en]', '$site_url', '$_POST[brand_desc]', '$_POST[brand_desc_en]', '$img_name', '$is_show', '$_POST[sort_order]')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT brand_id, brand_name, brand_name_en, site_url, brand_logo, brand_desc, brand_desc_en, brand_logo, is_show, sort_order ".
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    /* 处理图片 */
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'brandlogo'));
    $param = "brand_name = '$_POST[brand_name]', brand_name_en = '$_POST[brand_name_en]',  site_url='$site_url', brand_desc='$_POST[brand_desc]', brand_desc_en='$_POST[brand_desc_en]', is_show='$is_show', sort_order='$_POST[sort_order]' ";
    if (!empty($img_name))
    {
        //有图片上传
...
elseif ($_REQUEST['act'] == 'edit_brand_name_en')
{
    check_authz_json('brand_manage');

    $id     = intval($_POST['id']);
    $name   = json_str_iconv(trim($_POST['val']));
	if ($exc->edit("brand_name_en = '$name'", $id))
	{
		admin_log($name,'edit','brand');
		make_json_result(stripslashes($name));
	}
