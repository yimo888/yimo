...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_value_en, v.attr_price ".
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        {
            $html .= '中文：<input name="attr_value_list[]" type="text" value="' .htmlspecialchars($val['attr_value']). '" size="20" /> ';
			$html .= '英文：<input name="attr_value_en_list[]" type="text" value="' .htmlspecialchars($val['attr_value_en']). '" size="20" /> ';
        }
        elseif ($val['attr_input_type'] == 2)
        {
            $html .= '中文：<textarea name="attr_value_list[]" rows="3" cols="40">' .htmlspecialchars($val['attr_value']). '</textarea>';
			$html .= '英文：<textarea name="attr_value_en_list[]" rows="3" cols="40">' .htmlspecialchars($val['attr_value_en']). '</textarea>';
        }
        else
        {
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT goods_id, goods_name, goods_name_en, goods_type, goods_sn, virtual_sales, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
