...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /*插入数据*/
    $sql = "INSERT INTO ".$ecs->table('pack')."(pack_name,pack_name_en, pack_fee, free_money, pack_desc, pack_desc_en, pack_img)
            VALUES ('$_POST[pack_name]','$_POST[pack_name_en]', '$_POST[pack_fee]', '$_POST[free_money]', '$_POST[pack_desc]', '$_POST[pack_desc_en]', '$img_name')";
    $db->query($sql);
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT * FROM " .$ecs->table('pack'). " WHERE pack_id='$_REQUEST[id]'";
    $pack = $db->GetRow($sql);
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $param = "pack_name = '$_POST[pack_name]',pack_name_en = '$_POST[pack_name_en]', pack_fee = '$_POST[pack_fee]', free_money= '$_POST[free_money]', pack_desc = '$_POST[pack_desc]', pack_desc_en = '$_POST[pack_desc_en]' ";
...
if ($_REQUEST['act'] == 'edit_name_en')
{
    check_authz_json('pack');

    $id = intval($_POST['id']);
    $val = json_str_iconv(trim($_POST['val']));
	$exc->edit("pack_name_en='$val'", $id);
	admin_log($val, 'edit', 'pack');
	make_json_result(stripslashes($val));
